...
在左侧条目列表区域右键 → 选择"添加条目"
输入条目名称（如："我的文档备份"）
选择同步模式（建议新手选择 B-增量同步）
点击确定
2.2 配置同步源和目标
在左侧列表中单击刚创建的条目
在右侧"源目录/文件"文本框中输入要同步的路径，每行一个：
...
源文件：\\192.168.3.101\发货\跟单大师\aaa.txt
目标目录：E:\sync_远程
最终路径：E:\sync_远程\192.168.3.101\发货\跟单大师\aaa.txt
📝 服务器名可以是 IP 地址（如 192.168.3.101）或主机名（如 myserver），处理方式相同。
